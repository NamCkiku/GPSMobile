...
namespace BA_Mobile.GoogleMaps.Helpers
{
    /// <summary>
    ...
    public static class GeoHelper
    {
        ...
        public static List<T> UpdateAddress<T>(List<T> list, List<string> listAddress, string addressFielName)
        {
            // Nếu list bị null hoặc rỗng thì không cần xử lý gì cả?
            if (list == null || list.Count == 0 || listAddress == null || listAddress.Count == 0)
            {
                return list;
            }
            var listTemp = list.ToArray();
            try
            {
                // Duyệt danh sách lấy địa chỉ
                if (listAddress.Count == listTemp.Length)
                {
                    for (int index = 0; index < listTemp.Length; index++)
                    {
                        var add = listTemp[index].GetType().GetProperty(addressFielName);
                        add.SetValue(listTemp[index], Convert.ChangeType(listAddress[index], add.PropertyType), null);
                    }
                }
            }
            catch (Exception)
